...
# find User pool id
USER_POOL_ID=$(aws cognito-idp list-user-pools --profile icebreaker --max-results 60 \
...
if [ -z "$USER_POOL_ID" ]; then
  ...
  USER_POOL_ID=$(aws cognito-idp create-user-pool --profile icebreaker --cli-input-json file://edit/$userPoolFileName | jq -r '.UserPool.Id')
...
USER_POOL_CLIENT_ID=$(aws cognito-idp list-user-pool-clients --profile icebreaker  --user-pool-id $USER_POOL_ID \
...
if [ -z "$USER_POOL_CLIENT_ID" ]; then
  ...
  USER_POOL_CLIENT_ID=$(aws cognito-idp create-user-pool-client --profile icebreaker  --cli-input-json file://edit/$userPoolClientFileName | jq -r '.UserPoolClient.ClientId')
